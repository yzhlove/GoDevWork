...
func (p *Packet) ReadString() (ret string, err error) {
	...
	ret = string(p.data[p.pos : p.pos+int(size)])
	p.pos += int(size)
	return
}

func (p *Packet) ReadS8() (int8, error) {
	if ret, err := p.ReadByte(); err != nil {
		return 0, err
	} else {
		return int8(ret), nil
	}
...
func (p *Packet) ReadS24() (int32, error) {
	if ret, err := p.ReadU24(); err != nil {
		return 0, err
	} else {
