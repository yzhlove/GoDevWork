...
var config = hystrix.CommandConfig{
	Timeout:                5000, //执行command的超时时间
	MaxConcurrentRequests:  8,    //command的最大并发量
	SleepWindow:            1000, //过多长时间，熔断器再次检测是否开启。(ms)
	ErrorPercentThreshold:  30,   //错误率  请求数量大于等于RequestVolumeThreshold并且错误率到达这个百分比就会启动
	RequestVolumeThreshold: 5,    //请求阀值(一个统计窗口10s内请求数量)熔断器是否打开首先要满足这个条件；这里的设置表示至少有5个请求才进行ErrorPercentThreshold的错误百分比计算
}
...
func (s *Hystrix) Run(name string, run runFunc) error {
	...
	if err := hystrix.Do(name, func() error {
		return run()
